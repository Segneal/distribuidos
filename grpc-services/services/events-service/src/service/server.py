"""
gRPC Server for Events Service
"""
import os
import sys
import grpc
...
from evento_service import EventoServiceImpl
...
def serve():
    ...
    events_pb2_grpc.add_EventoServiceServicer_to_server(EventoServiceImpl(), server)
    ...
    # Start server
    server.start()
    logger.info(f"Events Service gRPC server started on {server_address}")
    
    try:
        server.wait_for_termination()
    except KeyboardInterrupt:
        logger.info("Shutting down Events Service gRPC server...")
        kafka_client.stop()
        server.stop(0)
